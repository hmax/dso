		...
		ImmaturePointTemporaryResidual* residuals)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		if(fh != point->host)
		{
			...
			residuals[nres].state_state = ResState::_IN;
	...
	int numGoodRes=0;
	for(int i=0;i<nres;i++)
		if(residuals[i].state_state == ResState::_IN) numGoodRes++;
